  ...
  const renderEvents = () => {
    ...
    return Object.keys(groupedEvents).map((year) => (
      <div key={year} className="mt-6">
        <h2 className="text-2xl font-bold text-blue-600">Events in {year}</h2>
          ...
          <div key={month} className="mt-4">
            ...
            <ul className="mt-2 space-y-2">
              {groupedEvents[year][month].map((event) => (
                <li key={event.id} className="p-4 border rounded-lg shadow-md bg-white hover:bg-gray-50 transition">
                  <Link href={`/update/${event.id}`} >
                  <div className="flex justify-between items-center mb-2">
                    <span className="text-xl font-medium text-gray-900">{event.title}</span>
